...
export default function SubmitResultsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [me, raceList] = await Promise.all([
          getMe(token),
          getRaces(),
        ])
  ...
  // Load candidates when race changes
  useEffect(() => {
    if (!selectedRaceId) {
      setCandidates([])
      return
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    // Validate all candidates have a vote number
    const voteEntries = candidates.map((c) => {
      const v = votes[c.id]
      if (v === '' || v === undefined) {
        throw new Error(`Enter votes for ${c.name}`)
      }
      return { candidateId: c.id, votes: parseInt(v, 10) }
    })
    ...
    try {
      ...
      await submitResults(token, {
        ...
        totalRegistered: totalRegistered ? parseInt(totalRegistered, 10) : undefined,
        totalVoted: totalVoted ? parseInt(totalVoted, 10) : undefined,
        rejectedBallots: rejectedBallots ? parseInt(rejectedBallots, 10) : 0,
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <select
            ...
            className="w-full px-4 py-3.5 text-base border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
            required
          >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-gray-300 rounded-xl text-lg focus:ring-2 focus:ring-blue-500 outline-none"
              placeholder="e.g. 520"
            />
          </div>
            ...
            {candidates.map((c) => (
              <div key={c.id} className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="w-24 px-3 py-3 text-lg text-center border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
                  placeholder="0"
                />
              </div>
            ))}
