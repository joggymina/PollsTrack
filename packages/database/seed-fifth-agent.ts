...
async function main() {
  console.log("Seeding fifth Agent + Polling Station...\n")
  ...
  // Fifth Polling Station
  let station = await prisma.pollingStation.findFirst({
    where: { code: "TEST-PS-005" },
  })
  if (!station) {
    station = await prisma.pollingStation.create({
      data: {
        code: "TEST-PS-005",
        name: "Test Church Polling Station",
        wardId: ward.id,
        registeredVoters: 390,
      },
    })
  ...
  console.log("\n========================================")
  console.log("FIFTH AGENT READY (CLEAN OFFLINE TEST)")
  console.log("========================================")
  console.log(`Phone to login     : 254700000005`)
